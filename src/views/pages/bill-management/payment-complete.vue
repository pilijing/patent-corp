        ...
        >
          ...
          >
            <template #default="{ row  }">
              <el-button
                type="text"
                :disabled="row.payState !== paymentState.TO_BE_PAID"
...
export default defineComponent({
  ...
  components: {
    ...
    DrawerPaymentCompleteBill: () => import('./drawer-payment-complete-bill.vue'),
  },
  setup(props, context) {
    const { $prompt, $message } = context.root;
    const { getRegionName } = useRegionsInject();
    const { getCodeInfo, getEnumsInfo, getTypeName } = useLookupsInject();
    const billState = computed(() => getEnumsInfo('STATE'));
    ...
    const state = reactive({
      ...
      filter: {
        lawFirmId: null, // 代理机构
        regionCode: '', // 国家/地区
        batchState: paymentState.value.TO_BE_PAID, // 出账状态
      },
      filterColumns: [
        {
          dataIndex: 'lawFirmId',
    ...
    options.query = computed(() => ({
      ...state.filter,
      billStage: billState.value.PAYMENT_STATE || 8904,
      billStatus: 'payment-complete',
      isList: true,
    }));
    ...
    const onComplateAudit = (row) => {
      $prompt('支付单号', '填写支付单号', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        inputPattern: /.+/,
        inputErrorMessage: '请输支付单号',
      }).then(async ({ value }) => {
        try {
          const { putData } = useHttp('/patent-corp_api/batch/set-payment-number');
          await putData({
            id: row.id,
            paymentNumber: value,
          });
          $message.success('填写支付单号成功');
          fetchList();
        } catch (error) {
          if (error.response) {
