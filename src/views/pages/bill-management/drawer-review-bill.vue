      ...
      <div style="padding:0 20px">
        ...
        <div class="batch-bill-table">
          ...
          <dialog-turn-down
            ...
            :bills-download-list="billsDownloadList"
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const statusList = computed(() => getEnumsInfo('BILL_REVIEW_STATE'));
    const reviewStateList = computed(() => getEnumsInfo('REVIEW_STATE'));
    ...
    const pass = async () => {
      if (state.billsDownloadList) {
        ...
        const params = {
          billState: statusList.value.PASS_REVIEW,
          billIds,
    ...
    onMounted(async () => {
      const statusNum = statusList.value.PASS_REVIEW;
      const statusNums = statusList.value.REVIEW_FAILED;
      state.titleNum = props.batchObject.number;
      if (props.batchObject.drawerState === statusNum) {
        state.filter.checkState = statusNum;
      } else if (props.batchObject.drawerState === statusNums) {
        state.filter.checkState = statusNums;
      } else {
        state.filter.checkState = null;
      }
      const batchState = props.batchObject.checkState;
      if (batchState === reviewStateList.value.PASS_REVIEW || batchState === reviewStateList.value.REVIEW_FAILED) {
